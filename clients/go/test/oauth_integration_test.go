...
var kPublicPort nat.Port
var kAdminPort nat.Port

func init() {
	kPublicPort, _ = nat.NewPort("tcp", "4444")
	kAdminPort, _ = nat.NewPort("tcp", "4445")
}
...
func (s *oauthIntegrationTestSuite) SetupSuite() {
	req := testcontainers.GenericContainerRequest{
		ContainerRequest: testcontainers.ContainerRequest{
			...
			WaitingFor:   wait.ForAll(wait.ForListeningPort(kPublicPort), wait.ForListeningPort(kAdminPort)),
...
func (s *oauthIntegrationTestSuite) TestFetchOAuthToken() {
	...
	endpoint, err := s.getEndpoint(kPublicPort, "oauth2/token")
	s.Require().NoError(err)
...
func (s *oauthIntegrationTestSuite) validateToken(authHeaderValue string) {
	...
	endpoint, err := s.getEndpoint(kAdminPort, "oauth2/introspect")
	s.Require().NoError(err)

	body := []byte(`token=` + strings.TrimLeft(authHeaderValue, "Bearer "))
...
func (s *oauthIntegrationTestSuite) ensureOAuthClientExists() {
	endpoint, err := s.getEndpoint(kAdminPort, "clients")
	s.Require().NoError(err)
